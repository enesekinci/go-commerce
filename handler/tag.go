...
func GetTags(c *fiber.Ctx) error {
	...
	return c.JSON(fiber.Map{"status": "success", "message": "Tags found", "tags": tags})
}
...
func CreateTag(c *fiber.Ctx) error {
	...
	return c.JSON(fiber.Map{"status": "success", "message": "Tag created", "tag": tag})
}
...
func DeleteTag(c *fiber.Ctx) error {
	...
	if tag.Name == "" {
		return c.Status(fiber.StatusNotFound).JSON(fiber.Map{"status": "error", "message": "No tag found with ID", "data": nil})
	}

	database.DB.Delete(&tag)

	return c.JSON(fiber.Map{"status": "success", "message": "Tag deleted", "tag": tag})
}
